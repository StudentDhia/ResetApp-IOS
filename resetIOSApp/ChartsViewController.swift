...
class ChartsViewController: UIViewController {
    ...
    fileprivate func barsChart(horizontal: Bool) -> Chart {
        ...
        let yModel = ChartAxisModel(firstModelValue: 0, lastModelValue: Double(self.nbrOfcig+1), axisTitleLabels: [ChartAxisLabel(text: "Number Of Cigarette", settings: labelSettings.defaultVertical())], axisValuesGenerator: generator, labelsGenerator: labelsGenerator)
    ...
    func getSmoking() {
        ...
        let url = "http://41.226.11.243:10080/reset/webservices/SmokingDataSource.php?id="+ViewController.user.token
        Alamofire.request(url, method: .get).responseJSON {
            ...
            case .success:
                ...
                if response.result.value != nil {
                    ...
                    let dict2 = dict["smoking_statistics"] as! NSArray
                    print(dict2)
                    
                    
                    for  i in  0..<dict2.count  {
                        //let p = NewsDoctor()
                        // bech yrajilna ken el contenu elli fil key main
                        let main:Dictionary=dict2[i] as! Dictionary<String,AnyObject>
                        print(main)
                        ...
                        if (self.nbrOfcig<Int(donnee)!){
                            self.nbrOfcig = Int(donnee)!
                        }
                       // self.Newslist.append(p)
                        
                    }
                   }
                //let main:Dictionary=dict[i] as! Dictionary<String,AnyObject>
